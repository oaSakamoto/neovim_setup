return {
  {
    ...
    config = function (_, opts)
      ...
      harpoon_map('<leader>', function() harpoon_picker(harpoon:list()) end, 'Abrir harpoon')
      harpoon_map('ha', function() harpoon:list():add() end, 'Adiciona buffer')
      harpoon_map('hc', function() harpoon:list():clear() end, 'Limpar lista')
      harpoon_map('hh', function() harpoon:list():prev() end, 'Buffer anterior')
      harpoon_map('hl', function() harpoon:list():next() end, 'Proximo Buffer')

      -- Mapeamentos para seleção de buffers
      local buffer_keys = { '(', '{', '[', '=' }
      local buffer_names = { 'Primeiro', 'Segundo', 'Terceiro', 'Quarto' }
      for i, key in ipairs(buffer_keys) do
        harpoon_map(key, function() harpoon:list():select(i) end, buffer_names[i] .. ' buffer')
      end

      -- Mapeamentos para substituição de buffers
      for i, key in ipairs(buffer_keys) do
        harpoon_map('hr' .. key, function() harpoon:list():replace_at(i) end, 'Substitui o ' .. buffer_names[i]:lower() .. ' buffer')
      end
    end
  },
}
